...
from kcd_dice_game.game_logic import (
    Game,
...
class GameCLI:
    ...
    def bank_points(self, args: List[str]):
        ...
        try:
            # Store the current player and their turn score before banking
            banking_player = self.game.current_player
            points_to_bank = banking_player.turn_score
            
            # Bank the points (this will change the current player)
            self.game.bank()
            
            # Display information about the player who just banked points
            print(f"Banked {points_to_bank} points for {banking_player.name}")
            print(f"{banking_player.name}'s total score: {banking_player.total_score}")

            # Check if game is over
            if self.game.is_game_over:
                # Since there's no get_winner method, we'll use the banking player as the winner
                # when game is over
                print(
                    f"\nGame over! {banking_player.name} wins with {banking_player.total_score} points!"
                )
        except GameStateException as e:
    ...
    def show_state(self, args: List[str]):
        ...
        # Access turn score from the current player's data in the players list
        current_player_data = next(
            (p for p in state["players"] if p["name"] == state["current_player"]), None
        )
        current_turn_score = (
            current_player_data["turn_score"] if current_player_data else 0
        )
